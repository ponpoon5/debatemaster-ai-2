  ...
  switch (framework) {
    ...
    case ThinkingFramework.FIVE_WHYS:
      frameworkDetails = `
            ...
            【ユーザー入力】
            ユーザーは「Problem（問題文）」のみを入力します。
            例：「出荷ミスが増えている」

            【あなた（AI）の役割】
            ユーザーが入力したProblemを受け取ったら、まずあなたが以下の「問題定義ブロック」を補完・生成してください：

            A. 問題定義ブロック（AIが生成）
              1. Problem（問題文）- ユーザー入力をそのまま表示
              2. Impact（影響/損失）- AIが推測して記述
                 例：「返品コスト増、顧客満足低下、信頼性低下」
              3. Scope（範囲）- AIが推測して記述
                 例：「営業部/受注工程/全商品カテゴリ」
              4. Timeframe（期間）- AIが推測して記述
                 例：「先月から顕著/週2-3回発生/直近10件中5件」
              5. Evidence（観測事実）- AIが推測して記述
                 例：「エラーログ増加、顧客クレーム3件、返品率5%上昇」

            この問題定義ブロックを出力した上で、「さあ、Why分析の5段階を入力しろ！」と指示してください。

            B. Why分析ブロック（ユーザーが入力）
              Why 1 → Why 2 → Why 3 → Why 4 → Why 5
              各段階で「なぜそうなったのか」を論理的に掘り下げる

            **ねらい**: Why チェーンの土台を「事実」寄りにして、精神論化を防ぐ。`;
      scoringCriteria = `
            [採点基準 (100点満点)]
            1. 因果の論理性 (50点): 「AだからB」という繋がりが強固か。各Whyが前のWhyに対する妥当な原因か。
            2. 深掘りの度合い (25点): 5段階まで深く掘り下げられているか。表面的でなく本質に迫っているか。
            3. 真因への到達度 (25点): 解決可能な根本原因に辿り着いたか。「人の意識」で終わらず、仕組み・プロセスの問題を指摘できているか。

            注意: 問題定義ブロック（Impact/Scope/Timeframe/Evidence）は採点対象外です。これらはAIが生成するため、ユーザーの回答には含まれません。`;
      break;
